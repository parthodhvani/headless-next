...
.map-location-slider::-webkit-scrollbar {
  width: 6px;
}

.map-location-slider::-webkit-scrollbar-thumb {
  background: #7c3aed;
  border-radius: 20px;
}
...
.map-location-card__info {
  display: flex;
  flex-direction: column;
  gap: 10px;
}

.map-location-card__info p {
  color: #94a3b8;
  line-height: 1.7;
}

.map-location-card__info a {
  color: #00f0ff;
      ...
      `}</style>
            ...
            <main>
                {sections.map((section, index) => {
                   ...
                   /* ── MAPS SECTION ── */
                   /* ── MAPS SECTION ── */
                   if (section.acf_fc_layout === "maps") {

                      const locations = section.locations || [];

                      if (!locations.length) return null;

                      // active location per section
                      const activeLocation =
                         activeLocations[index] || locations[0];

                     

                      return (
                         <section key={index} className="map-section">
                            ...
                            <div className="map-section__grid">
                               ...
                               <div className="map-section__map-wrap">
                                  ...
                                  {activeLocation?.address && (
                                     ...
                                     <iframe
                                        ...
                                        title={activeLocation.location_title || "Map"}
                               ...
                               <div className="map-section__locations">
                                  ...
                                  <div className="map-location-slider">

                                     {locations.map((location: any, i: number) => (

                                        <div
                                           key={i}
                                           className={`map-location-card ${activeLocation?.location_title === location.location_title
                                                 ? "map-location-card--active"
                                                 : ""
                                              }`}
                                           onClick={() =>
                                              setActiveLocations((prev: any) => ({
                                                 ...prev,
                                                 [index]: location,
                                              }))
                                           }
                                        >
                                           ...
                                           <div className="map-location-card__top">
                                              <h3>{location.location_title}</h3>
                                              <span>📍</span>
                                           </div>
